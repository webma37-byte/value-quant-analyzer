...
## 5. 최종 투자 판단
STRONG BUY / BUY / HOLD / AVOID 중 하나를 선택하고, "최종 판단: [등급]" 형식으로 명시하세요. 핵심 근거 3~5개, 주요 리스크 3개, 포트폴리오 비중 권고를 포함합니다.

분석 시 반드시 정량적 데이터(숫자)를 포함하고, 주관적 판단은 최소화하세요. 한국어로 작성합니다.`;
    ...
    const response = await fetch("https://api.anthropic.com/v1/messages", {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: "user",
            content: "회사명: " + company.trim() + "\n\n위 회사에 대해 밸류 퀀트 5단계 심층 분석을 수행해주세요. 반드시 웹 검색으로 최신 데이터를 수집한 후 분석하세요.",
          },
        ],
      }),
    });
